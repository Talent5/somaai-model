...
class ScholarshipRecommender:
    def __init__(self, firebase_credentials: Dict[str, Any], scholarship_data_path: str):
        self.db = self._setup_firestore(firebase_credentials)
    ...
    def _setup_firestore(self, firebase_credentials: Dict[str, Any]) -> firestore.Client:
        try:
    ...
    def process_users(self, min_score: float = 0.15) -> None:
        try:
            ...
            logging.info(f"Processing recommendations for {total_users} users:")
            ...
            for user in users:
                ...
                logging.info(f"User: {first_name} {last_name}")
                logging.info(f"User ID: {user_id}")
                logging.info(f"Number of matched scholarships: {num_matches}")
            ...
            logging.info("\nSummary:")
            logging.info(f"Total users processed: {processed_users}")
            logging.info(f"Total scholarships matched: {total_scholarships}")
            logging.info(f"Average scholarships per user: {avg_scholarships:.2f}")
